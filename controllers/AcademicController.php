...
class AcademicController {
    ...
    public function courseSubjects() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_subject_name'])) {
            ...
            if ($nombre !== '') {
                ...
                // Verificar si aún hay horas disponibles antes de agregar
                $validation = $this->courseModel->canAssignHours($courseId, $sid, 1);
                if (!$validation['allowed']) {
                    header('Location: ?action=academic&open_subjects='.$courseId.'&error=hours_full&available='.$validation['available'].'&assigned='.$validation['currentTotal']);
                    exit;
                }
                $this->linkSubjectToCourse($courseId, $sid);
            }
    ...
    public function setSubjectHours() {
        ...
        // Validar que el nuevo total no supere la capacidad semanal
        $validation = $this->courseModel->canAssignHours($courseId, $subjectId, $hours);
        if (!$validation['allowed']) {
            $maxDisp = $validation['available'];
            $yaUsadas = $validation['currentTotal'] - ($validation['currentTotal'] - $maxDisp + ($hours - ($validation['remaining'] - 0)));
            $msg = "No se pueden asignar {$hours} horas: el total sería {$validation['newTotal']} pero el límite semanal es {$validation['available']} horas. "
                 . "Horas disponibles para esta asignatura: {$validation['remaining']}.";
            header('Location: ?action=academic&open_subjects=' . $courseId . '&error=hours_exceeded&msg=' . urlencode($msg));
            exit;
        }
