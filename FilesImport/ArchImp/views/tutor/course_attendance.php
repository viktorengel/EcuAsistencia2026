...
function renderStats(s, total) {
    ...
    if (!s) return;
    var t   = parseInt(total) || 0;
    var pre = parseInt(s.presente)    || 0;
    var aus = parseInt(s.ausente)     || 0;
    var tar = parseInt(s.tardanza)    || 0;
    var jus = parseInt(s.justificado) || 0;
    function pct(v){ return t > 0 ? Math.round(v/t*1000)/10 : 0; }
    function bar(v,c){ return '<div class="progress-bar"><div class="progress-fill" style="width:'+pct(v)+'%;background:'+c+';"></div></div>'; }
    sec.innerHTML =
        '<div class="stat-card total"><div class="number">'+t+'</div><div class="label">Total registros</div></div>' +
        '<div class="stat-card presente"><div class="number">'+pre+'</div><div class="label">Presentes ('+pct(pre)+'%)</div>'+bar(pre,'#28a745')+'</div>' +
        '<div class="stat-card ausente"><div class="number">'+aus+'</div><div class="label">Ausentes ('+pct(aus)+'%)</div>'+bar(aus,'#dc3545')+'</div>' +
        '<div class="stat-card tardanza"><div class="number">'+tar+'</div><div class="label">Tardanzas ('+pct(tar)+'%)</div>'+bar(tar,'#ffc107')+'</div>' +
        '<div class="stat-card justif"><div class="number">'+jus+'</div><div class="label">Justificados ('+pct(jus)+'%)</div>'+bar(jus,'#17a2b8')+'</div>';
}
